        ...
        def sql_for_field_with_date_range(field, operator, value, db_table, db_field, is_custom_filter=false)
          sql=case operator
            when "t>"
                ...
                ("#{db_table}.#{db_field} > '%s'" % [self.class.connection.quoted_date(dt.end_of_day)])
            when "t<"
                dt=get_date_range_from_string(value.first.strip)
                ("#{db_table}.#{db_field} < '%s'" % [self.class.connection.quoted_date(dt.at_beginning_of_day)])
            when "t><"
                ("#{db_table}.#{db_field} BETWEEN '%s' AND '%s'" % [self.class.connection.quoted_date(( Date.parse(value[0]) rescue Date.today ).to_time.at_beginning_of_day), self.class.connection.quoted_date(( Date.parse(value[1]) rescue Date.today ).to_time.end_of_day)])
            when "tm="
              from=Time.now.at_beginning_of_month.months_ago(value.first.to_i*-1)
              to=from.at_end_of_month
              ("#{db_table}.#{db_field} BETWEEN '%s' AND '%s'" % [self.class.connection.quoted_date(from), self.class.connection.quoted_date(to)])
            when "tw="
              from=Time.now.at_beginning_of_week+(value.first.to_i).week
              to=Time.now.at_end_of_week+(value.first.to_i).week
              ("#{db_table}.#{db_field} BETWEEN '%s' AND '%s'" % [self.class.connection.quoted_date(from), self.class.connection.quoted_date(to)])
            when "=r"
              ("#{db_table}.#{db_field} RLIKE '#{self.class.connection.quote_string(value.first.to_s)}'")
            else
